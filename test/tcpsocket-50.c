...
/*
 * medusa_tcpsocket_connect_with_options() must never deliver an event from
 ...
 * tcpsocket pointer. a caller that tears down its context from the error
...
/* set while medusa_tcpsocket_connect_with_options() is on the stack */
static int g_inconnect;
/* set once the caller has destroyed the tcpsocket */
...
static int tcpsocket_onevent (struct medusa_tcpsocket *tcpsocket, unsigned int events, void *context, void *param)
{
        ...
        if (g_destroyed != 0 &&
            (events & MEDUSA_TCPSOCKET_EVENT_DESTROY) == 0) {
                g_nevents_afterdestroy += 1;
        }
        if (events & MEDUSA_TCPSOCKET_EVENT_ERROR) {
                g_nerrors += 1;
                g_error = medusa_tcpsocket_get_error(tcpsocket);
                fprintf(stderr, "      state: %d, error: %d\n", medusa_tcpsocket_get_state(tcpsocket), g_error);
        }
        return 0;
}
...
static int connect_options_default (struct medusa_tcpsocket_connect_options *options, struct medusa_monitor *monitor)
{
        int rc;
        rc = medusa_tcpsocket_connect_options_default(options);
        if (rc < 0) {
                fprintf(stderr, "medusa_tcpsocket_connect_options_default failed, rc: %d\n", rc);
                return rc;
        }
        options->monitor        = monitor;
        options->onevent        = tcpsocket_onevent;
...
static int test_rewritten_address (unsigned int poll)
{
        ...
        rc = connect_options_default(&tcpsocket_connect_options, monitor);
        if (rc < 0) {
                goto bail;
        }
        tcpsocket_connect_options.protocol      = MEDUSA_TCPSOCKET_PROTOCOL_ANY;
        tcpsocket_connect_options.address       = "loopback";
        tcpsocket_connect_options.port          = 12345;

        tcpsocket = medusa_tcpsocket_connect_with_options(&tcpsocket_connect_options);
        if (MEDUSA_IS_ERR_OR_NULL(tcpsocket)) {
                fprintf(stderr, "medusa_tcpsocket_connect_with_options failed, error: %d\n", MEDUSA_PTR_ERR(tcpsocket));
                goto bail;
        }
        ...
        /*
         * nothing is listening on the port, so a connection error is fine. a
         * resolver error is not - it means the deferred work was handed the
         * name the caller passed in rather than the rewritten address.
         */
        if (g_error == ENOENT) {
...
static int test_connect_resolved_failure (unsigned int poll)
{
        ...
        tcpsocket_connect_options.protocol      = MEDUSA_TCPSOCKET_PROTOCOL_IPV4;
        tcpsocket_connect_options.address       = "127.0.0.1";
        tcpsocket_connect_options.port          = 12345;
        tcpsocket_connect_options.sprotocol     = MEDUSA_TCPSOCKET_PROTOCOL_IPV4;
        tcpsocket_connect_options.saddress      = "1.2.3.4";
...
static int test_unresolvable_address (unsigned int poll)
{
        ...
        tcpsocket_connect_options.protocol      = MEDUSA_TCPSOCKET_PROTOCOL_ANY;
        tcpsocket_connect_options.address       = "medusa-test-nonexistent-host.invalid";
        tcpsocket_connect_options.port          = 80;
...
/*
 * the caller may destroy the tcpsocket between connect() returning and the
