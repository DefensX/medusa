...
/* every descriptor that is ready when the backend polls must be dispatched
...
static int test_poll (unsigned int poll)
{
        ...
        /* backend->add is applied by the monitor on its next iteration */
        ...
        if (total != NIOS) {
                fprintf(stderr, "poll: %d dispatched %d of %d ready ios in one pass\n", poll, total, NIOS);
                goto bail;
        }
