...
/* every ready io destroys itself from its own callback. a backend that
 * dispatches by walking the live registration array while del() mutates it
 * shifts the array under the loop index and skips whichever entry slides into
 * the vacated slot, so only about half of the ios are dispatched in the pass.
 * all of them must be. */
...
static int test_poll (unsigned int poll)
{
        ...
        if (total != NIOS) {
                fprintf(stderr, "poll: %d dispatched %d of %d self destroying ios in one pass\n", poll, total, NIOS);
                goto bail;
        }
